...
- **Rapid QR Check-in & Method Tracking**: When a teacher scans a student's QR code during a lesson session, the backend resolves the token to the student, verifies group membership, and creates an `attendance_records` row with `status = 'PRESENT'` and `recording_method = 'QR_SCAN'` if no record exists. If already recorded, the existing record remains unmodified.
...
- **Exclusivity Constraint & Concurrency Safety**: The composite unique constraint `uq_session_student` on `(session_id, student_id)` acts as the ultimate physical arbiter of concurrency. Under concurrent identical scan requests, it guarantees physical deduplication at the database storage level, allowing the application layer to catch race constraint violations and return the winner record idempotently.
