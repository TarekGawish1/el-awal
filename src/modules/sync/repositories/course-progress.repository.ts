...
export class CourseProgressRepository {
   ...
   * 1. Position Monotonicity: last_position_seconds = GREATEST(existing.pos, incoming.pos)
   * 2. Completion Monotonicity: is_completed = (existing.is_completed OR incoming.is_completed)
   * 3. Completed Timestamp Lock: completed_at is locked at first completion; older offline events cannot erase it.
   */
   ...
   * Computes and returns the updated course completion percentage upon commit.
   */
  async syncBatch(
    studentId: string,
    ...
    // Execute atomic sync batch inside a PostgreSQL transaction
    ...
    // Rollup overall course progress percentage leveraging covering index: idx_course_progress_rollup
    const progressStats = await this.calculateCourseProgressPercentage(studentId, courseId);

    this.logger.log(
      `Synced ${processedIds.length} offline progress operations for student ${studentId} in course ${courseId}. New progress: ${progressStats}%`,
    );
   ...
   * Formula: (Completed Lessons Count / Total Lessons in Course) * 100
   */
