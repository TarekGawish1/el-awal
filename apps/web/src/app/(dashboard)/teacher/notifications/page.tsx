...
export default function NotificationCenterPage() {
  ...
  const { data: feedData, isLoading: isFeedLoading, refetch: refetchFeed } = useNotifications();
  ...
  const filteredNotifications = notifications.filter((item) => {
    ...
    if (filterType === 'READ' && !item.isRead) return false;
    if (searchQuery.trim()) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12" dir="rtl">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                إدارة تشغيل وإيقاف قنوات الإرسال (واتساب، إشعارات المتصفح، داخل المنصة) على مستوى النظام
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-100/80 p-1 rounded-xl self-stretch md:self-auto">
          ...
          >
            <Power size={14} />
            <span>قواطع القنوات والإعدادات</span>
          </button>
          <button
      ...
      {activeTab === 'controls' ? (
        <div className="space-y-6">
          {/* Master System Switches Section */}
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-6">
            <div className="border-b border-slate-100 pb-4">
              <h2 className="text-base font-bold text-slate-900">
                التحكم التفصيلي في فئات الإشعارات التلقائية
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                يمكنك تخصيص وتحديد أنواع الإشعارات التلقائية التي ترغب بإرسالها للمستخدمين
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center bg-slate-100 p-1 rounded-xl text-xs">
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-lg font-bold transition-all ${
                    filterType === 'UNREAD' ? 'bg-white text-blue-600 shadow-xs' : 'text-slate-500'
                  }`}
                >
            ...
            {isFeedLoading ? (
              ...
              <div className="py-16 text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  لم يتم العثور على أي إشعارات مطابقة لمعايير البحث الحالية.
                </p>
              </div>
            ) : (
