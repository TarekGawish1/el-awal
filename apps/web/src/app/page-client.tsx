...
function IntroSequence({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#0a0f1c] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
      dir="rtl"
      initial={{ y: "0%" }}
      exit={{ y: "-100%" }}
      ...
      <div className="relative z-10 container mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          <a
            ...
            className="text-blue-600 font-bold transition-colors"
          >
            الرئيسية
          </a>
          <a
                ...
                {isLoginDropdownOpen && (
                  ...
                  >
                    <div className="py-1">
                      <Link
                        href="/login"
                        className="block px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-blue-600 transition-colors"
                      >
                        دخول كطالب
                      </Link>
                      <Link
                      ...
                      <Link
                        href="/parent-access"
                        className="block px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-blue-600 transition-colors border-t border-slate-100"
                      >
                        دخول كولي أمر
                      </Link>
                    </div>
...
function loadHeroFramesOnce(): Promise<HTMLImageElement[]> {
  ...
  heroFramesLoadPromise = (async () => {
    const results: (HTMLImageElement | null)[] = new Array(
      HERO_FRAME_COUNT,
    ).fill(null);
    for (let start = 0; start < HERO_FRAME_COUNT; start += CONCURRENCY) {
      ...
      const loaded = await Promise.all(
        batch.map((index) => loadSingleHeroFrame(index)),
      );
      loaded.forEach((image, offset) => {
        results[start + offset] = image;
      });
    }
    cachedHeroFrames = results.filter(
...
function HeroImageSequence() {
    ...
    const startLoading = () => {
      ...
      const scheduleIdle = () => {
        ...
        if (
          ...
          "requestIdleCallback" in (window as any)
        ) {
  ...
  return (
    <div className="absolute inset-0 z-0 pointer-events-none">
      <canvas
        ref={canvasRef}
        className="block w-full h-full opacity-10"
        aria-hidden="true"
      />
    </div>
  );
      ...
      <div className="flex-1 relative z-10 container mx-auto px-6 pt-32 pb-16 flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8">
        ...
        >
          <h1 className="flex flex-col gap-4 text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 mb-8 leading-normal">
            <span className="text-blue-600">الاستاذ/ أحمد غريب</span>
          </h1>

          <p className="text-lg sm:text-xl text-slate-600 mb-12 leading-loose max-w-lg font-medium">
            منهجية مبسطة وتدريبات مكثفة تضمن لك التفوق وفهم الرياضيات بكل سهولة.
            انضم الآن.
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center lg:justify-start gap-5 w-full">
            <Link
              href="/login"
              className="bg-blue-600 hover:bg-blue-700 text-white px-10 py-4 rounded-full font-bold text-lg shadow-lg shadow-blue-500/30 transition-transform hover:-translate-y-1 inline-block"
            >
              ابدأ التعلم الآن
            </Link>
            <button
              ...
              className="bg-white hover:bg-slate-50 text-slate-700 border-2 border-slate-200 px-8 py-4 rounded-full font-bold text-lg flex items-center gap-3 transition-colors"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
...
const COURSES = [
  {
    ...
    description:
      "شرح مبسط ومفصل لمنهج الرياضيات للصف الأول الثانوي مع تدريبات مكثفة وتطبيقات على النظام الجديد.",
    badge: "ترم أول",
  ...
  },
  {
    ...
    description:
      "تأسيس قوي في التفاضل وحساب المثلثات والجبر مع حل أسئلة بنك المعرفة وامتحانات سابقة.",
    badge: "ترم أول",
  ...
  },
  {
    ...
    description:
      "مراجعات نهائية وتدريبات متقدمة للثانوية العامة تضمن لك الدرجة النهائية في جميع فروع الرياضيات.",
    badge: "علمي رياضة",
    ...
    if (course.modules?.length) {
      for (const mod of course.modules) {
        const match = mod.lessons?.find(
          (l: any) => l.isPreview && l.freeVideoUrl,
        );
        if (match) {
          freeLesson = {
            id: match.id,
            title: match.title,
            videoUrl: match.freeVideoUrl,
          };
          break;
        }
      }
    }
    if (!freeLesson && course.freeVideoUrl) {
      freeLesson = {
        id: course.freeVideoLessonId || "free-1",
        title: "معاينة الدرس المجاني",
        videoUrl: course.freeVideoUrl,
      };
    }
      ...
      <AnimatePresence>
        {selectedCourse && (
          ...
          >
            ...
            >
              {/* Header */}
              <div className="bg-slate-900 text-white p-6 sm:p-8 flex items-start justify-between shrink-0 border-b border-slate-800">
                <div className="max-w-2xl">
                  <div className="flex flex-wrap items-center gap-2.5 mb-3">
                    ...
                    {selectedCourse.subject && (
                      <span className="bg-indigo-900/60 text-indigo-300 border border-indigo-700/50 text-xs font-semibold px-3 py-1 rounded-full">
                        مادة {selectedCourse.subject}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
                {/* Video & Subscription Sidebar */}
                <div className="w-full lg:w-[380px] bg-slate-50 border-b lg:border-b-0 lg:border-l border-slate-200 p-6 flex flex-col shrink-0 order-1 lg:order-2 overflow-y-auto">
                  {activePreviewLesson?.videoUrl ? (
                    <div className="mb-4">
                      <div className="w-full aspect-video bg-black rounded-2xl overflow-hidden shadow-lg mb-3 relative group border border-slate-200">
                        <iframe
                          key={
                            activePreviewLesson.id +
                            activePreviewLesson.videoUrl
                          }
                      ...
                      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center gap-2.5 text-xs text-emerald-800 font-semibold shadow-xs">
                        <PlayCircle className="w-4 h-4 text-emerald-600 shrink-0" />
                        <span className="truncate">
                          جاري تشغيل: {activePreviewLesson.title}
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="w-full aspect-video bg-slate-900 rounded-2xl flex flex-col items-center justify-center p-6 text-center shadow-inner mb-4 text-slate-300 border border-slate-800">
                      <PlayCircle className="w-12 h-12 text-slate-500 mb-2" />
                      <p className="text-xs font-semibold text-slate-400">
                        اختر درساً متاحاً للمعاينة المجانية من القائمة
                      </p>
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-baseline gap-3">
                      ...
                      <span className="text-sm font-semibold text-slate-400 line-through">
                        {Math.round(selectedCourse.price * 1.5)} ج.م
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 leading-relaxed">
                        يتيح لك الاشتراك فتح كافة الدروس، المذكرات، والاختبارات
                        التفاعلية
                      </p>
                    </div>
                    ...
                    <Link
                      href="/login"
                      className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all shadow-md hover:shadow-lg text-sm"
                    >
                    ...
                    <div className="pt-4 border-t border-slate-200 space-y-3">
                      ...
                      <ul className="text-xs text-slate-600 space-y-2.5">
                        ...
                        <li className="flex items-center gap-2.5">
                          ...
                          <span>مذكرات وملخصات PDF لكل درس</span>
                        </li>
                        <li className="flex items-center gap-2.5">
                          <ClipboardList className="w-4 h-4 text-blue-600 shrink-0" />
                          <span>اختبارات إلكترونية تفاعلية وواجبات</span>
                        </li>
                      </ul>
                    </div>
                  </div>
                ...
                <div className="w-full lg:flex-1 p-6 sm:p-8 overflow-y-auto order-2 lg:order-1 bg-white">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <span className="text-xs font-bold text-slate-600 bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200">
                      {selectedCourse.modules?.length || 0} وحدات •{" "}
                  ...
                  selectedCourse.modules.length > 0 ? (
                    <div className="space-y-3.5">
                      {selectedCourse.modules.map(
                        (mod: any, moduleIndex: number) => {
                          const isExpanded =
                            expandedModule === (mod.id || moduleIndex);
                          return (
                            <div
                              key={mod.id || moduleIndex}
                            ...
                            >
                              <button
                                ...
                                onClick={() =>
                                  setExpandedModule(
                                    isExpanded ? null : mod.id || moduleIndex,
                                  )
                                }
                              ...
                              >
                                ...
                                <span className="text-xs font-bold text-slate-600 bg-white border border-slate-200 px-2.5 py-1 rounded-full shrink-0 shadow-2xs">
                                  {mod.lessons?.length || 0} دروس
                                </span>
                              </button>
                                              ...
                                              <div
                                                ...
                                                onClick={() => {
                                                  if (isFree) {
                                                    ...
                                                    toast.success(
                                                      `تم تشغيل المعاينة المجانية: ${lesson.title}`,
                                                    );
                                                  } else {
                                                    toast(
                                                      "هذا الدرس متاح حصرياً للمشتركين في الكورس",
                                                      {
                                                        icon: "🔒",
                                                      },
                                                    );
                                                ...
                                                className={`flex items-center justify-between p-3.5 rounded-xl transition-all cursor-pointer ${
                                                  isPlaying
                                                    ...
                                                    : isFree
                                                      ? "bg-white hover:bg-emerald-50/50 border border-slate-200 hover:border-emerald-300"
                                                      : "bg-white/60 hover:bg-white border border-slate-100 opacity-75"
                                                }`}
                                              >
                                                <div className="flex items-center gap-3 min-w-0">
                                                  {isFree ? (
                                                    <PlayCircle
                                                      className={`w-5 h-5 shrink-0 ${isPlaying ? "text-emerald-600 animate-pulse" : "text-emerald-500"}`}
                                                    />
                                                  ) : (
                                                  ...
                                                  <div className="min-w-0">
                                                    <div
                                                      className={`text-sm font-bold truncate ${isPlaying ? "text-emerald-950" : isFree ? "text-slate-900" : "text-slate-600"}`}
                                                    >
                                                      الدرس{" "}
                                                      {lesson.orderIndex ||
                                                        lessonIndex + 1}
                                                ...
                                                {isFree ? (
                                                  ...
                                                  <span className="text-[11px] font-medium text-slate-400 bg-slate-100 px-2.5 py-1 rounded-md shrink-0">
                                                    خاص بالمشتركين
                                                  </span>
                                                )}
                                              </div>
                    ...
                    <div className="p-8 text-center bg-slate-50 rounded-2xl border border-slate-100">
                      <p className="text-slate-500 text-sm font-medium">
                        لم يتم إضافة وحدات لهذا الكورس بعد.
                      </p>
                    </div>
                  )}
                </div>
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          >
            ...
            <p className="text-slate-600 text-lg font-medium">
              اختر المرحلة الدراسية الخاصة بك وابدأ رحلة التفوق مع أقوى محتوى
              تعليمي في الرياضيات.
            </p>
          </motion.div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-500">
              لم يتم إضافة كورسات تطابق اختيارك في الوقت الحالي.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {courses.map((course, index) => (
              ...
              >
                ...
                <div
                  className={`h-48 w-full bg-gradient-to-br ${course.color || "from-blue-600 to-cyan-600"} relative overflow-hidden flex items-start justify-start p-6`}
                >
                  ...
                  <div className="relative z-10 flex items-center gap-2">
                    <span className="bg-white/25 backdrop-blur-md px-3.5 py-1 rounded-full text-white text-xs font-bold shadow-xs">
                      {course.badge}
                    </span>
                    {course.totalLessons > 0 && (
                      <span className="bg-black/30 backdrop-blur-md px-3 py-1 rounded-full text-white text-xs font-semibold">
                ...
                {/* Content Area */}
                <div className="p-6 sm:p-7 flex-1 flex flex-col bg-white">
                  <div className="flex items-center gap-2 mb-2 text-xs text-slate-500 font-medium">
                    ...
                    {course.gradeLevel && course.teacherName && <span>•</span>}
                    {course.teacherName && <span>{course.teacherName}</span>}
                  </div>

                  <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2.5 leading-snug">
                    {course.title}
                  </h3>
                  ...
                  <div className="flex items-baseline gap-2 mb-5 mt-auto pt-3 border-t border-slate-100">
                  ...
                  <div className="flex flex-col gap-2.5">
                    <button
                      type="button"
                      onClick={() => handleOpenCourseModal(course)}
                      className="hover:bg-emerald-50 flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 border-emerald-200 text-emerald-700 font-bold transition-colors group-hover:border-emerald-300 text-sm shadow-2xs"
                    >
                      <PlayCircle className="h-4 w-4" />
                      <span>
                        {course.hasFreeVideo
                          ? "شاهد المعاينة المجانية وتفاصيل المنهج"
                          : "عرض تفاصيل ومحتوى الكورس"}
                      </span>
                    </button>
                    <Link
                      href="/login"
                      className="hover:bg-blue-50 flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 border-slate-100 text-slate-700 font-bold transition-colors group-hover:border-blue-200 text-sm"
                    >
                      <span>اشترك الآن</span>
                      <ArrowRight className="h-4 w-4 rtl:rotate-180 text-blue-600 transition-transform group-hover:-translate-x-1" />
                    </Link>
                  </div>
                </div>
              </motion.div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          >
            ...
            <p className="text-slate-600 text-lg font-medium">
              اختر المرحلة والصف الدراسي لمعرفة مواعيد وأماكن المحاضرات الحضورية
              المتاحة.
            </p>
          </motion.div>
        </div>
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden border border-slate-100">
          ...
          <div className="p-6 sm:p-8 min-h-[250px]">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-400 py-10">
                ...
                <p className="text-lg font-medium">
                  لا توجد مواعيد متاحة حالياً لهذا الصف
                </p>
              </div>
            ) : (
              <AnimatePresence mode="wait">
                ...
                >
                  {currentSchedules.map((item, index) => (
                    <div
                      key={index}
                      className="bg-slate-50 border border-slate-100 rounded-2xl p-6 hover:shadow-lg hover:border-blue-100 transition-all group"
                    >
            ...
            <div className="mt-10 text-center">
              <Link
                href="/login"
                className="bg-slate-900 hover:bg-blue-600 text-white px-10 py-4 rounded-full font-bold text-lg shadow-lg shadow-slate-900/20 hover:shadow-blue-600/30 transition-all hover:-translate-y-1 inline-flex items-center justify-center gap-3 w-full sm:w-auto"
              >
    ...
    const syncAndFetchCertificates = async () => {
      try {
        ...
        for (const cert of localCerts) {
          if (!syncedFlags[cert.id]) {
            try {
              ...
              formData.append(
                "stage",
                cert.stage || (cert.data && cert.data.stage) || "",
              );
              formData.append(
                "grade",
        ...
        if (uniqueSaved.length > 0) {
          const newSecondary = uniqueSaved.filter(
            (c: any) => c.stage === "الثانوية",
          );
          const newPreparatory = uniqueSaved.filter(
            (c: any) => c.stage === "الإعدادية",
          );
          const newPrimary = uniqueSaved.filter(
            (c: any) => c.stage === "الابتدائية",
          );
          const newOther = uniqueSaved.filter(
            (c: any) =>
              !["الثانوية", "الإعدادية", "الابتدائية"].includes(c.stage),
          );
        ...
        <div className="space-y-16">
          {visibleStages.length === 0 ? (
            <div className="text-center py-12 bg-white/50 backdrop-blur-sm rounded-2xl border border-slate-200 shadow-sm max-w-3xl mx-auto">
              ...
              <p className="text-slate-500">
                لم يتم إصدار أو مزامنة أي شهادات بعد. كن أول المتفوقين!
              </p>
            </div>
          ) : (
...
function loadAboutImagesOnce(): Promise<HTMLImageElement[]> {
  ...
  aboutImagesLoadPromise = Promise.all(
    [0, 1, 2].map(loadSingleAboutImage),
  ).then((first) => {
    cachedAboutImages = first.filter(
...
function AboutBackgroundSequence() {
  ...
  return (
    <div className="absolute inset-0 z-0 bg-slate-900 overflow-hidden">
      ...
      />
      <div
        className="absolute bottom-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full bg-indigo-600/15 blur-[120px]"
        aria-hidden="true"
      />
      <div
        className="absolute inset-0 bg-[url('/noise.svg')] opacity-20 mix-blend-overlay"
...
function AboutUsSection() {
  return (
    <section
      className="pt-16 sm:pt-24 relative overflow-hidden flex flex-col min-h-[85vh] sm:min-h-[80vh]"
      id="about"
      dir="rtl"
    >
          ...
          <div className="w-full max-w-4xl mx-auto mb-10">
            ...
            >
              <div className="bg-white/10 p-4 sm:p-8 rounded-3xl border border-white/20 backdrop-blur-xl shadow-2xl flex flex-col items-center justify-center">
                <div className="text-3xl sm:text-5xl font-black text-blue-400 mb-2 sm:mb-4">
                  +25
                </div>
                <div className="font-bold text-white text-sm sm:text-xl mb-1 text-center">
                  سنوات خبرة
                </div>
                <p className="text-xs sm:text-base text-slate-300 text-center hidden sm:block">
                  في تدريس الرياضيات
                </p>
              </div>
          ...
          <div className="w-full mt-auto -mb-8 sm:-mb-12">
            ...
            >
              ...
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-black mb-3 sm:mb-5 leading-tight">
                تعرف على الأستاذ /{" "}
                <span className="text-blue-400">أحمد غريب</span>
              </h2>

              <div className="space-y-3 sm:space-y-4 text-slate-200 text-[13px] sm:text-base md:text-lg leading-relaxed max-w-2xl mx-auto">
                <p>
                  نحن في منصة الأول نؤمن بأن الرياضيات ليست مجرد أرقام وقوانين،
                  بل هي لغة العقل ومنهج للتفكير. هدفنا هو تبسيط المادة وتوصيلها
                  للطلاب بأسهل وأمتع الطرق الممكنة.
                </p>
                <p>
                  الأستاذ <strong className="text-white">أحمد غريب</strong>،
                  خبير تدريس الرياضيات للمراحل الإعدادية والثانوية، يمتلك خبرة
                  واسعة في تأهيل الطلاب للتعامل مع النظام الحديث للامتحانات بثقة
                  واقتدار، من خلال التركيز على الفهم العميق والتدريب المستمر.
                </p>
              </div>
            </motion.div>
          </div>
    ...
    <form ref={formRef} className="space-y-4" onSubmit={handleSubmit}>
      <div>
        ...
        <input
          ...
          required
          className="w-full bg-slate-50 border border-slate-200 text-slate-900 px-4 py-3 rounded-xl outline-none focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all"
        />
      </div>
      <div>
        <label className="block text-sm font-bold text-slate-700 mb-2">
        ...
        <input
          ...
          dir="ltr"
          required
          className="w-full bg-slate-50 border border-slate-200 text-slate-900 px-4 py-3 rounded-xl outline-none focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all text-right"
        />
      </div>
      <div>
        <label className="block text-sm font-bold text-slate-700 mb-2">
        ...
        <textarea
          ...
          required
          className="w-full bg-slate-50 border border-slate-200 text-slate-900 px-4 py-3 rounded-xl outline-none focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all resize-none"
        ></textarea>
      </div>
      <button
        type="submit"
        disabled={isPending}
        className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold py-4 px-6 rounded-xl transition-all flex items-center justify-center gap-2"
      >
...
export default function RootPage() {
  ...
  if (isInitialized && isAuthenticated && user) {
    return (
      <div
        className="min-h-screen bg-[#0a0f1c] flex items-center justify-center text-slate-300 text-sm"
        dir="rtl"
      >
