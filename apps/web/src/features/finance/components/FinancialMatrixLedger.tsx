...
function PaymentCell({ student, kind, month, bookletId, academicYear, academicTerm }: { student: MatrixLedgerStudent; kind: 'TUITION' | 'BOOKLET'; month?: number; bookletId?: string; academicYear: string; academicTerm: 'FIRST_TERM' | 'SECOND_TERM' }) {
  ...
  return <span title={cell?.amountPaid ? `مدفوع جزئياً: ${formatAmount(cell.amountPaid)}` : 'غير مدفوع'} className="flex min-w-24 flex-col items-center gap-1"><XCircle className="h-5 w-5 text-rose-500" /><span className="text-[10px] font-bold text-rose-600">{cell?.amountPaid ? `مدفوع ${formatAmount(cell.amountPaid)}` : 'غير مدفوع'}</span></span>;
}
...
export function FinancialMatrixLedger({ groups = [] }: { groups?: any[] }) {
  ...
  const query = { gradeLevel: gradeLevel || undefined, academicPeriodId: `${academicYear}:${academicTerm}`, academicYear, academicTerm, groupId: groupId || undefined, stage: stage === 'ALL' ? undefined : stage, search: search || undefined, page, limit };
