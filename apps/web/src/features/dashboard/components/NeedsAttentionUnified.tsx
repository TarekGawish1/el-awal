...
import { FileCheck, UserX, Clock, UserPlus, CheckCircle2 } from 'lucide-react';
      ...
      <div className="space-y-3">
        {itemsToShow.map((item, index) => {
          if (item.type === 'reservation') {
            const res = item.data as any; // Type workaround for pending reservations
            return (
              <div key={`res-${res.id}`} className="group flex items-center justify-between p-3.5 bg-white border border-neutral-200 rounded-xl hover:border-neutral-300 transition-colors shadow-sm">
                ...
                <Link href="/teacher/reservations">
                  <Button size="sm" variant="outline" className="h-8 text-xs font-semibold bg-white">
                    مراجعة
                  </Button>
                </Link>
              </div>
          ...
          if (item.type === 'grading') {
            ...
            return (
              <div key={`grad-${grading.assessmentId}`} className="group flex items-center justify-between p-3.5 bg-white border border-neutral-200 rounded-xl hover:border-neutral-300 transition-colors shadow-sm">
                <div className="flex items-center gap-3">
                ...
                <Link href={`/teacher/assessments/${grading.assessmentId}/submissions`}>
                  <Button size="sm" variant="outline" className="h-8 text-xs font-semibold bg-white text-blue-700 hover:text-blue-800 border-blue-200 hover:bg-blue-50">
                    تصحيح
                  </Button>
                </Link>
              </div>
          ...
          if (item.type === 'risk') {
            ...
            return (
              <div key={`risk-${risk.id}`} className="group flex items-center justify-between p-3.5 bg-white border border-neutral-200 rounded-xl hover:border-neutral-300 transition-colors shadow-sm">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-error-50 flex items-center justify-center shrink-0">
                ...
                <Link href={`/teacher/students`}>
                  <Button size="sm" variant="ghost" className="h-8 text-xs font-semibold text-neutral-500 hover:text-neutral-800">
                    متابعة
                  </Button>
                </Link>
              </div>
