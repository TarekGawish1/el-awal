    ...
    <div className="flex flex-col items-center p-8 border-none rounded-3xl shadow-sm bg-gradient-to-b from-white to-slate-50 ring-1 ring-slate-100 relative overflow-hidden">
      ...
      {showConfirm ? (
        ...
        <div className="w-full space-y-3">
          ...
          {studentPhone && (
            <a
              href={`https://wa.me/${studentPhone.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`مرحباً ${data.fullName}، كود الطالب الخاص بك هو: ${data.studentCode}`)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center w-full rounded-xl bg-green-500 hover:bg-green-600 text-white font-medium py-3 px-4 transition-colors shadow-sm"
            >
