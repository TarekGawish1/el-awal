...
export const CertificateTemplateA = forwardRef<HTMLDivElement, Props>(
  ({ data }, ref) => {
    const isMale = data.gender === "MALE";
    const studentTitle = isMale ? "الطالب" : "الطالبة";
    const getGrammarText1 = () =>
      isMale ? "لأدائه المتميز وتفوقه" : "لأدائها المتميز وتفوقها";
    ...
    return (
      ...
      >
        ...
        >
          {/* Presenter Sentence */}
          <p
            className="text-[2.2rem] text-[#4A4A4A] font-bold"
            style={{ fontFamily: "'Amiri', serif" }}
          >
            يسر الأستاذ {data.teacherName} أن يمنح هذه الشهادة إلى{" "}
            {studentTitle}
          </p>
          ...
          <div className="max-w-[850px] text-center mt-4">
            <p
              className="text-[1.8rem] text-[#4A4A4A] leading-[1.8] font-bold"
              style={{ fontFamily: "'Amiri', serif" }}
            >
              وذلك تقديرًا {getGrammarText1()} العلمي الملحوظ في مادة
              <span className="text-[#1D4ED8] mx-2">
                {data.subject || "......"}
              </span>
        ...
        >
          <span
            className="text-[1.5rem] font-bold text-[#4A4A4A] leading-none"
