...
export function LessonEditorModal({
  ...
  const handleAddAttachment = async () => {
    if (!newAttachmentTitle.trim() || !newAttachmentUrl.trim()) {
      toast.error('يرجى كتابة عنوان ورابط الملف');
      return;
    }
    if (!lesson?.id) {
    ...
    try {
      await addAttachmentMutation.mutateAsync({
        ...
        data: {
          ...
          fileKey: `courses/attachments/${Date.now()}-${newAttachmentTitle.trim()}`,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden text-white my-auto flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/90">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">إعداد الشرح، الفيديو، الملخص والاختبارات التفاعلية</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex border-b border-slate-800 px-6 bg-slate-950/40 text-xs">
          ...
          >
            ...
            {lesson?.attachments && lesson.attachments.length > 0 && (
              <span className="bg-indigo-500/20 text-indigo-300 px-1.5 py-0.5 rounded-full text-[10px]">
        ...
        <div className="p-6 overflow-y-auto space-y-4 text-right flex-1">
          ...
          {activeTab === 'video' && (
            <div className="space-y-4">
              ...
              <div className="p-4 bg-slate-950/60 border border-slate-800 rounded-2xl space-y-3">
                ...
                <div className="border-2 border-dashed border-slate-800 hover:border-indigo-500/50 rounded-xl p-4 text-center cursor-pointer transition-colors relative">
                  <input
                    type="file"
                    accept="video/*"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-xs font-mono text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-xs text-white pl-10 focus:outline-none focus:border-indigo-500"
                    />
                ...
                <div className="flex items-center justify-between p-3.5 bg-slate-950 border border-slate-800 rounded-xl my-auto">
                  <div>
                    <p className="text-xs font-bold text-white">معاينة مجانية (Free Preview)</p>
                    <p className="text-[10px] text-slate-400">إتاحة مشاهدة هذا الدرس لجميع الزوار مجاناً</p>
                  </div>
                  <input
                    type="checkbox"
          ...
          {activeTab === 'summary' && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-indigo-400">يدعم تنسيق Markdown والعناوين</span>
              </div>
              <textarea
                rows={10}
                value={summary}
          ...
          {activeTab === 'attachments' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-slate-200">الملفات والمذكرات المرفقة بالدرس</h3>
                {lesson && !isAddingAttachment && (
                  <button
                    type="button"
                    onClick={() => setIsAddingAttachment(true)}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600/20 text-indigo-300 hover:bg-indigo-600 hover:text-white rounded-xl text-xs font-semibold transition-colors"
                  >
              ...
              {!lesson && (
                <div className="p-4 bg-amber-950/30 border border-amber-800/40 rounded-2xl text-xs text-amber-300">
              ...
              {isAddingAttachment && (
                <div className="p-4 bg-slate-950 border border-indigo-500/30 rounded-2xl space-y-3">
                  <p className="text-xs font-bold text-indigo-300">إضافة مرفق جديد للدرس</p>
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1">اسم المرفق (مثال: ملخص القوانين PDF)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1">رابط الملف المباشر (Cloudflare R2 / CDN)</label>
                    <input
                      type="url"
                      value={newAttachmentUrl}
                      onChange={(e) => setNewAttachmentUrl(e.target.value)}
                      placeholder="https://assets.elawal.com/courses/..."
              ...
              <div className="space-y-2">
                {(!lesson?.attachments || lesson.attachments.length === 0) ? (
                  <p className="text-xs text-slate-500 text-center py-6">لا توجد ملفات مرفقة بهذا الدرس حالياً</p>
                ) : (
        ...
        {/* Footer Actions */}
        <div className="px-6 py-4 border-t border-slate-800 flex justify-between items-center bg-slate-900/90">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            className="px-6 py-2.5 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-500 text-white transition-colors shadow-lg shadow-indigo-600/30 disabled:opacity-50 flex items-center gap-2"
          >
