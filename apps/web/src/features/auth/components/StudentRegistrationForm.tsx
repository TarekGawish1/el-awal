    ...
    const text = [
      ...
      ...(credentials.parentPhone
        ? [
            ...
            `• هاتف ولي الأمر: ${credentials.parentPhone}`,
            `• كلمة المرور: ${credentials.parentPassword || credentials.studentPassword}`,
          ]
        : []),
    ].join('\n');
    ...
    <div className="space-y-6 animate-in fade-in-50 duration-200 text-center" aria-live="polite">
      ...
      {credentials && (
        <div className="space-y-4 text-start">
          ...
          {/* Parent Credentials Card (if parentPhone provided) */}
          {credentials.parentPhone && (
            <div className="rounded-2xl border border-neutral-200 bg-neutral-50/60 p-4 sm:p-5 space-y-3 shadow-2xs">
              <div className="flex items-center justify-between border-b border-neutral-200 pb-2.5">
                <div className="flex items-center gap-2 text-neutral-900 font-extrabold text-sm">
                  ...
                  <span>بيانات حساب ولي الأمر (للمتابعة والدرجات)</span>
                </div>
                <span className="text-[11px] font-bold bg-neutral-200 text-neutral-700 px-2 py-0.5 rounded-full">
                  ولي الأمر
                </span>
              </div>

              <div className="space-y-2.5">
                <div className="flex items-center justify-between bg-white rounded-xl p-3 border border-neutral-200 shadow-2xs">
                  <div className="space-y-0.5">
                    <span className="text-[11px] font-semibold text-neutral-500 block">هاتف ولي الأمر</span>
                    <span className="font-mono text-sm font-bold text-neutral-900" dir="ltr">
                      {credentials.parentPhone}
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={() => handleCopy(credentials.parentPhone, 'parentPhone')}
                    className="flex items-center gap-1 text-xs font-bold text-neutral-600 hover:text-neutral-900 bg-neutral-100 hover:bg-neutral-200 px-2.5 py-1.5 rounded-lg transition-colors"
                  >
                    {copied === 'parentPhone' ? <Check className="h-3.5 w-3.5 text-emerald-600" /> : <Copy className="h-3.5 w-3.5" />}
                    <span>{copied === 'parentPhone' ? 'تم النسخ' : 'نسخ'}</span>
                  </button>
                </div>

                <div className="flex items-center justify-between bg-white rounded-xl p-3 border border-neutral-200 shadow-2xs">
                  <div className="space-y-0.5">
                    <span className="text-[11px] font-semibold text-neutral-500 block">كلمة مرور ولي الأمر</span>
                    <span className="font-mono text-sm font-bold text-neutral-900" dir="ltr">
                      {credentials.parentPassword || credentials.studentPassword}
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={() => handleCopy(credentials.parentPassword || credentials.studentPassword, 'parentPassword')}
                    className="flex items-center gap-1 text-xs font-bold text-neutral-600 hover:text-neutral-900 bg-neutral-100 hover:bg-neutral-200 px-2.5 py-1.5 rounded-lg transition-colors"
                  >
                    {copied === 'parentPassword' ? <Check className="h-3.5 w-3.5 text-emerald-600" /> : <Copy className="h-3.5 w-3.5" />}
                    <span>{copied === 'parentPassword' ? 'تم النسخ' : 'نسخ'}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <span>{copied === 'all' ? 'تم نسخ جميع البيانات بنجاح ✅' : 'نسخ جميع بيانات الحساب دفعة واحدة'}</span>
          </Button>
          ...
          <div className="rounded-xl bg-emerald-50 border border-emerald-200/80 p-3.5 flex items-start gap-3 text-xs text-emerald-900 leading-relaxed">
            ...
            <div>
              <p className="font-bold text-emerald-950">إشعار واتساب التلقائي:</p>
              <p className="text-emerald-800 text-[11px] mt-0.5">
                سيتم أيضاً إرسال هذه البيانات كاملة في رسالة واتساب لرقم ولي الأمر فور قيام المعلم بقبول وتأكيد الحجز. 📲
              </p>
            </div>
          </div>
        </div>
