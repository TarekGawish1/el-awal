...
import { z } from 'zod';
import { X, BookOpen, Loader2, Layers, DollarSign, Package } from 'lucide-react';
...
type CreateBookletFormData = z.infer<typeof createBookletSchema>;
...
export function CreateBookletModal({ isOpen, onClose, groups = [] }: Props) {
  const { createBooklet, isCreating } = useBooklets();
  ...
  const {
    ...
    watch,
    setValue,
  } = useForm<CreateBookletFormData>({
    resolver: zodResolver(createBookletSchema),
    defaultValues: {
      title: '',
      price: 50,
  ...
  const onSubmit = async (data: CreateBookletFormData) => {
    try {
      await createBooklet({
        ...
        stockCount: data.stockCount ? Number(data.stockCount) : null,
      });

      toast.success('تمت إضافة المذكرة الدراسية بنجاح 📚');
      reset();
      onClose();
    } catch (err: any) {
      toast.error(err.message || 'حدث خطأ أثناء إضافة المذكرة');
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-purple-50 to-indigo-50/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900">إضافة مذكرة / ملزمة دراسية</h2>
              <p className="text-xs text-slate-500">إنشاء مذكرة وربطها بالصف والمجموعات للتحصيل السريع</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          <div>
            <Label htmlFor="title" className="text-slate-700 font-medium mb-1.5 flex items-center gap-1.5">
              <BookOpen className="w-4 h-4 text-purple-600" />
              عنوان أو اسم المذكرة <span className="text-red-500">*</span>
            </Label>
            <Input
              id="title"
              placeholder="مثال: مذكرة الشرح والتدريبات - الفصل الأول"
              {...register('title')}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="price" className="text-slate-700 font-medium mb-1.5 flex items-center gap-1.5">
            ...
            <div>
              <Label htmlFor="stockCount" className="text-slate-700 font-medium mb-1.5 flex items-center gap-1.5">
                <Package className="w-4 h-4 text-amber-600" />
                عدد النسخ المتاحة (اختياري)
              </Label>
              <Input
                id="stockCount"
                type="number"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="gradeLevel" className="text-slate-700 font-medium mb-1.5 flex items-center gap-1.5">
            ...
            <div>
              <Label htmlFor="groupId" className="text-slate-700 font-medium mb-1.5">
                المجموعة المستهدفة
              </Label>
              <select
                id="groupId"
